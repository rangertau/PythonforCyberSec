...
def WMIProcessCreation(name):
	c = wmi.WMI()
	processID,returnValue = c.win32_Process.Creat(CommandLine=name)
	print("Process %s created with PID %d" & (name, processID))

def PSProcessCreation(name):
	command = ["powershell", "& { invoke-wmimethod win32_process -name create -argumentlist notepad.exe \ |select ProcessID | % {$_ProcessId} | Write-Host }"]
