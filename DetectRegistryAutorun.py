...
def printResults(hive,path,autoruns):
	...
	for autorun in autoruns:
		print("t%s: %s" % (autorun[0],autorun[1]))
	print()

hives = {
	"HKCU": winreg.HEKY_CURRENT_USER,
	"HKLM": winreg.HKEY_LOCAL_MACHINE}
paths = ["SOFTWARE\Microsot\Windows\CurrentVersion\Run",
...
def checkAutoruns():
	...
	numKeys = winreg.QueryInfoKey(winreg.KEY_USERS)[0]
